...
const TnbCheckbox = () => {
    ...
    useEffect(() => {
        ...
        api.get(EP_GET_TRAINER)
            ...
            .then(res => res.accounts.find((acc: AccountType) => acc.type === userTypes.TRAINER))
                        ...
                        <Styles className={'form-cbx__wrapper'}>
                            <div className={'form-cbx__cont'}>
                                <input className={'form-cbx__input'} type={'checkbox'} {...field}
                                       checked={field.value}/>
                                <span className={'form-cbx__label'}>{t('auth:accept-tnb')} <a target={'_blank'}
                                                                                              href={tnb}>{t('auth:tnb')}</a></span>
                            </div>
                            <FormError name={'tnb'}/>
                        </Styles>
                    )
